...
class Chrome
{
    ...
    {
        if (!file_exists($chromePath)) throw new \Exception("路徑不存在：{$chromePath}");

        $filepath = $chromePath . DIRECTORY_SEPARATOR . 'style';
        
        if (!file_exists($filepath)) throw new \Exception("請建立路徑：{$filepath}");
    ...
    protected static function put(string $chromeFilename, callable $callback): string
    {
        ...
        if (!file_exists($realFileName)) throw new \Exception("模板樣式不存在：{$realFileName}");
